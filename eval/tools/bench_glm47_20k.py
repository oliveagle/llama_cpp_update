...
TESTS = [
    {"name": "20k 长文本", "msg": "请详细介绍 Python 编程语言，包括其历史、特点、应用场景和主要特性，每个部分至少500字，总计2000字。", "max_tokens": 20480, "repeat": 2},
]

async def do_test(session, test):
    try:
        start = time.time()
        async with session.post(CHAT_URL, json={"model": "glm-4.7-flash", "messages": [{"role": "user", "content": test["msg"]}], "max_tokens": test["max_tokens"]}, timeout=600) as resp:
            if resp.status != 200:
                return None
            first = time.time()
...
async def main():
    async with aiohttp.ClientSession() as session:
        print("=" * 50)
        print("GLM-4.7-Flash 20K 长文本性能测试")
        print("=" * 50)
        ...
        for idx, t in enumerate(TESTS, 1):
            ...
            for j in range(t["repeat"]):
                print(f"  开始 {j+1}/{t['repeat']}...", end="", flush=True)
                r = await do_test(session, t)
                if r and r["success"]:
                    results.append(r)
                    all_ps.append(r["ps"])
                    all_gs.append(r["gs"])
                    print(f"  OK! 总:{r['total']:.1f}s 首字节:{r['first_time']:.1f}s 生成:{r['gen_time']:.1f}s {r['ct']}t {r['gs']:.1f}t/s")
                else:
                    print(f"  FAIL")
            if results:
