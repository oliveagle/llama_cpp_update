"""
Inference for Block-wise Quantized Model (Final Attempt)
"""
...
class BlockwiseLinear:
    """Linear layer with block-wise quantization"""

    def __init__(self, quants, scales, mins, block_size, bias=None):
    ...
    def _reconstruct_weight(self):
        ...
        out_f, num_blocks, block_size = self.quants.shape

        blocks = []
        for block_idx in range(num_blocks):
            block = self.quants[:, block_idx].float() * self.scales[:, block_idx].unsqueeze(1) + self.mins[:, block_idx].unsqueeze(1)
            blocks.append(block)
...
class BlockwiseModel:
    def __init__(self, model_path):
        ...
        print(f"Loading block-wise model from {model_path}...")
    ...
    def _reconstruct(self, nq):
        ...
        mins = nq['mins'].float()
        out_f, num_blocks, block_size = quants.shape

        blocks = []
        for block_idx in range(num_blocks):
            block = quants[:, block_idx].float() * scales[:, block_idx].unsqueeze(1) + mins[:, block_idx].unsqueeze(1)
            blocks.append(block)
    ...
    def _build_linear(self, name):
        ...
        if weight_name in self.quantized:
            ...
            if data.get("type") == "blockwise":
                nq = data["data"]
                return BlockwiseLinear(nq['quants'], nq['scales'], nq['mins'], nq['block_size'], bias)
...
if __name__ == "__main__":
    ...
    parser.add_argument("--model", default="models/qwen3-0.6b-blockwise-q4.gguf")
    parser.add_argument("--prompt", default="Hello, my name is")
    parser.add_argument("--max-tokens", type=int, default=30)
    args = parser.parse_args()

    model = BlockwiseModel(args.model)
