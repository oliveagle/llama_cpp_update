"""
Inference for 2-bit Quantized Model
"""
...
class TwoBitLinear:
    ...
    def __init__(self, centroids, indices, bias=None):
        self.centroids = centroids.float()  # [out_f, 4]
        self.indices = indices.long()  # [out_f, num_blocks, block_size]
        self.bias = bias.float() if bias is not None else None
        self.out_features = centroids.shape[0]
    ...
    def _reconstruct_weight(self):
        ...
        out_f = self.centroids.shape[0]
        num_blocks = self.indices.shape[1]
        block_size = self.indices.shape[2]

        # Gather centroids based on indices
        expanded = self.centroids.unsqueeze(1).unsqueeze(2).expand(-1, num_blocks, block_size, -1)
        gathered = torch.gather(expanded, 3, self.indices.unsqueeze(-1)).squeeze(-1)
...
class TwoBitModel:
    def __init__(self, model_path):
        ...
        print(f"Loading 2-bit model from {model_path}...")
    ...
    def _get_tensor(self, name):
        if name in self.quantized:
            ...
            if data.get("type") == "2bit":
                nq = data["data"]
                return self._reconstruct_2bit(nq)
            else:
                return data["data"]
        raise KeyError(f"Tensor {name} not found")

    def _reconstruct_2bit(self, nq):
        """Reconstruct weight from 2-bit data"""
        centroids = nq['centroids'].float()
        indices = nq['indices'].long()
        out_f = centroids.shape[0]
        num_blocks = indices.shape[1]
        block_size = indices.shape[2]

        expanded = centroids.unsqueeze(1).unsqueeze(2).expand(-1, num_blocks, block_size, -1)
    ...
    def _build_linear(self, name):
        ...
        if weight_name in self.quantized:
            ...
            if data.get("type") == "2bit":
                nq = data["data"]
                return TwoBitLinear(nq['centroids'], nq['indices'], bias)
            elif data.get("type") == "original":
                # Handle FP16 original weights
...
if __name__ == "__main__":
    ...
    parser.add_argument("--model", default="models/qwen3-0.6b-2bit.gguf")
    parser.add_argument("--prompt", default="Hello, my name is")
    parser.add_argument("--max-tokens", type=int, default=30)
    args = parser.parse_args()

    model = TwoBitModel(args.model)
