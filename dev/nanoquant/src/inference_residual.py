"""
Inference for Residual Binary Quantized Model
"""
...
class ResidualLinear:
    ...
    def __init__(self, B1_list, B2_list, scales_list, bias=None):
        self.B1_list = [b.float() for b in B1_list]
        self.B2_list = [b.float() for b in B2_list]
        self.scales_list = [s.float() for s in scales_list]
        self.bias = bias.float() if bias is not None else None
        self.out_features = B1_list[0].shape[0]
    ...
    def forward(self, x):
        ...
        output = None
        for B1, B2, scales in zip(self.B1_list, self.B2_list, self.scales_list):
            x_proj = torch.matmul(x_2d, B2)
            term_output = torch.matmul(x_proj, B1.T)
            term_output = term_output * scales.unsqueeze(0)

            if output is None:
                output = term_output
            else:
                output = output + term_output
...
class ResidualModel:
    def __init__(self, model_path):
        ...
        print(f"Loading residual model from {model_path}...")
    ...
    def _build_linear(self, name):
        ...
        if weight_name in self.quantized:
            ...
            if data.get("type") == "residual":
                nq = data["data"]
                B1_list = [t['B1'] for t in nq['terms']]
                B2_list = [t['B2'] for t in nq['terms']]
                scales_list = [t['scales'] for t in nq['terms']]
                bias = self.quantized[bias_name]["data"] if bias_name in self.quantized else None
                return ResidualLinear(B1_list, B2_list, scales_list, bias)
...
if __name__ == "__main__":
    ...
    parser.add_argument("--model", default="models/qwen3-0.6b-residual-3term.gguf")
    parser.add_argument("--prompt", default="Hello, my name is")
    parser.add_argument("--max-tokens", type=int, default=30)
    args = parser.parse_args()

    model = ResidualModel(args.model)
