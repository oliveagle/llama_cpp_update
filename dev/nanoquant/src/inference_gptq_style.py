"""
Inference for GPTQ-style Quantized Model
"""
...
class WeightedQuantLinear:
    """Linear layer with importance-weighted quantized weights"""

    def __init__(self, centroids, indices, num_bits, bias=None):
...
class GPTQStyleModel:
    def __init__(self, model_path):
        ...
        print(f"Loading GPTQ-style model from {model_path}...")
    ...
    def _get_tensor(self, name):
        if name in self.quantized:
            ...
            if data.get("type") == "3bit_weighted":
                nq = data["data"]
                return self._reconstruct(nq)
            else:
                return data["data"]
    ...
    def _build_linear(self, name):
        ...
        if weight_name in self.quantized:
            ...
            if data.get("type") == "3bit_weighted":
                nq = data["data"]
                return WeightedQuantLinear(nq['centroids'], nq['indices'], nq['num_bits'], bias)
...
if __name__ == "__main__":
    ...
    parser.add_argument("--model", default="models/qwen3-0.6b-gptq-style.gguf")
    parser.add_argument("--prompt", default="Hello, my name is")
    parser.add_argument("--max-tokens", type=int, default=30)
    args = parser.parse_args()

    model = GPTQStyleModel(args.model)
