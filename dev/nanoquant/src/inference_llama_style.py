"""
Inference for llama.cpp-style Q4_K_M quantized model
"""
...
class Q4KMLinear:
    """Linear layer with Q4_K_M style quantization"""

    def __init__(self, quants, scales, mins, block_size, super_block_size, bias=None):
        self.quants = quants.long()
        self.scales = scales.float()
        self.mins = mins.float()
        self.block_size = block_size
        self.super_block_size = super_block_size
        self.bias = bias.float() if bias is not None else None
        self.out_features = quants.shape[0]

        # Precompute weight
        self.weight = self._reconstruct_weight()

    def _reconstruct_weight(self):
        """Reconstruct weight from Q4_K_M quantization"""
        out_f, num_super_blocks, _ = self.quants.shape
        blocks_per_super = self.super_block_size // self.block_size
        ...
        for super_idx in range(num_super_blocks):
            ...
            for block_idx in range(blocks_per_super):
                ...
                block_quants = self.quants[:, super_idx, start:end]
                block = block_quants * self.scales[:, super_idx, block_idx].unsqueeze(1) + self.mins[:, super_idx, block_idx].unsqueeze(1)
...
class Q4KMModel:
    def __init__(self, model_path):
        ...
        print(f"Loading Q4_K_M style model from {model_path}...")
    ...
    def _get_tensor(self, name):
        if name in self.quantized:
            ...
            if data.get("type") == "q4_k_m":
                return self._reconstruct(data["data"])
            else:
                return data["data"]
        raise KeyError(f"Tensor {name} not found")

    def _reconstruct(self, nq):
        """Reconstruct weight"""
        quants = nq['quants'].long()
        scales = nq['scales'].float()
        mins = nq['mins'].float()
        block_size = nq['block_size']
        super_block_size = nq['super_block_size']
    ...
    def _build_linear(self, name):
        ...
        if weight_name in self.quantized:
            ...
            if data.get("type") == "q4_k_m":
                nq = data["data"]
                return Q4KMLinear(nq['quants'], nq['scales'], nq['mins'],
                                 nq['block_size'], nq['super_block_size'], bias)
            elif data.get("type") == "original":
                return FP16Linear(data["data"], bias)
...
if __name__ == "__main__":
    ...
    parser.add_argument("--model", default="models/qwen3-0.6b-q4-k-m-style.gguf")
    parser.add_argument("--prompt", default="Hello, my name is")
    parser.add_argument("--max-tokens", type=int, default=30)
    args = parser.parse_args()

    model = Q4KMModel(args.model)
