#!/bin/bash
# Qwen3VL-4B RoPE 缩放服务器
set -e

_SROOT="$(cd "$(dirname "$(realpath "$0")")" && pwd -P)"
SERVER_BIN="/home/oliveagle/opt/llama.cpp/build/bin/llama-server"
PORT=8401
LOG_FILE="$_SROOT/logs/qwen3vl-4b-rope.log"
PID_FILE="$_SROOT/logs/qwen3vl-4b-rope.pid"

MODEL_PATH="/mnt/volume3/modelscope_models/Qwen/Qwen3-VL-4B-Instruct-GGUF/Qwen3VL-4B-Instruct-Q8_0.gguf"
ROPE_SCALE=4.0
...
start() {
    ...
    log_info "启动 Qwen3VL-4B + RoPE (32K->128K)..."
