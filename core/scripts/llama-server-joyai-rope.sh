#!/bin/bash
# JoyAI-LLM-Flash 服务器 (原生128K支持)
set -e

_SROOT="$(cd "$(dirname "$(realpath "$0")")" && pwd -P)"
SERVER_BIN="/home/oliveagle/opt/llama.cpp/build/bin/llama-server"
PORT=8401
LOG_FILE="$_SROOT/logs/joyai-rope.log"
PID_FILE="$_SROOT/logs/joyai-rope.pid"

MODEL_PATH="/mnt/volume3/modelscope_models/yairpatch/JoyAI-LLM-Flash-GGUF/JoyAI-LLM-Flash-Q4_K_M.gguf"
# 原生128K, 无需RoPE缩放
...
start() {
    ...
    log_info "启动 JoyAI-LLM-Flash (原生128K)..."
    log_info "模型: DeepSeek2架构, 28GB, 原生支持128K context"
...
status() {
    echo "=========================================="
    echo "JoyAI-LLM-Flash Server (端口 $PORT)"
    echo "=========================================="
    if is_running; then
