#!/bin/bash
#!/bin/bash
# JoyAI-LLM-Flash 服务器 (32K context)
set -e

_SROOT="$(cd "$(dirname "$(realpath "$0")")" && pwd -P)"
SERVER_BIN="/home/oliveagle/opt/llama.cpp/build/bin/llama-server"
PORT=8401
LOG_FILE="$_SROOT/logs/joyai-32k.log"
PID_FILE="$_SROOT/logs/joyai-32k.pid"
...
start() {
    ...
    log_info "启动 JoyAI-LLM-Flash (32K context)..."
    log_info "模型: 28GB, DeepSeek2架构"
...
status() {
    echo "=========================================="
    echo "JoyAI-LLM-Flash 32K Server (端口 $PORT)"
    echo "=========================================="
    if is_running; then
