#!/bin/bash
# MiniCPM-o-4.5 RoPE 缩放服务器 (原生 40K)
set -e

_SROOT="$(cd "$(dirname "$(realpath "$0")")" && pwd -P)"
SERVER_BIN="/home/oliveagle/opt/llama.cpp/build/bin/llama-server"
PORT=8401
LOG_FILE="$_SROOT/logs/minicpm-o-rope.log"
PID_FILE="$_SROOT/logs/minicpm-o-rope.pid"

MODEL_PATH="/mnt/volume3/modelscope_models/OpenBMB/MiniCPM-o-4_5-gguf/MiniCPM-o-4_5-Q4_K_M.gguf"
...
start() {
    ...
    log_info "启动 MiniCPM-o-4.5 (原生 40K, 使用 32K)..."
    log_info "模型: Qwen3架构, 4.7GB"
...
status() {
    echo "=========================================="
    echo "MiniCPM-o-4.5 RoPE Server (端口 $PORT)"
    echo "=========================================="
    if is_running; then
