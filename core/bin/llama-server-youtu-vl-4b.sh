#!/bin/bash
# Youtu-VL-4B-Instruct 服务器
set -e

_SROOT="$(cd "$(dirname "$(realpath "$0")")" && pwd -P)"
SERVER_BIN="/home/oliveagle/opt/llama.cpp/build/bin/llama-server"
PORT=8401
LOG_FILE="$_SROOT/logs/youtu-vl-4b.log"
PID_FILE="$_SROOT/logs/youtu-vl-4b.pid"

MODEL_PATH="/mnt/volume3/modelscope_models/Tencent-YouTu-Research/Youtu-VL-4B-Instruct-GGUF/Youtu-VL-4B-Instruct-Q8_0.gguf"
...
start() {
    ...
    log_info "启动 Youtu-VL-4B (32K)..."
    log_info "模型: 4B 参数, Q8_0 量化"
...
status() {
    echo "=========================================="
    echo "Youtu-VL-4B Server (端口 $PORT)"
    echo "=========================================="
    if is_running; then
