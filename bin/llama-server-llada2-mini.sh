#!/bin/bash
# LLaDA2.0-mini-preview 服务器
set -e

_SROOT="$(cd "$(dirname "$(realpath "$0")")" && pwd -P)"
SERVER_BIN="/home/oliveagle/opt/llama.cpp/build/bin/llama-server"
PORT=8401
LOG_FILE="$_SROOT/logs/llada2-mini.log"
PID_FILE="$_SROOT/logs/llada2-mini.pid"
...
status() {
    echo "=========================================="
    echo "LLaDA2.0-mini Server (端口 $PORT)"
    echo "=========================================="
    if is_running; then
